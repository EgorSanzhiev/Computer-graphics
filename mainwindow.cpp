    ...
    : QMainWindow(parent)
{
    ...
    connect(loadFileAction, SIGNAL(triggered(bool)), this, SLOT(loadImage()));

    loadFileAction->trigger();
...
{
    QString filename = QFileDialog::getOpenFileName(this, tr("Choose file"), "", tr("Images (*.png *.jpg)"));

    Controller::getInstance()->setImageName(filename);
}
